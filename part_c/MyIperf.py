...
def update_router_config(net):
    routers = [router1, router2, router3, router4]
    buf_size = '10kb'
    for router in routers:
        for interface in router['interfaces']:
            net[router['name']].cmd(
                'sudo tc qdisc add dev {} root handle 1: tbf rate 100mbit buffer {} limit {}'.format(interface['name'],
                                                                                                     buf_size,
                                                                                                     buf_size))
            net[router['name']].cmd(
                'sudo tc qdisc add dev {} parent 1:1 handle 10: netem delay 30ms'.format(interface['name']))
